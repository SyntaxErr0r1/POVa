...
class SegmentationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.sam is None:
            ...
            input_size = tuple(self.target_size)
            original_size = tuple(original_size)
            print(f"input_size: {input_size[0]}")
...
def collate_fn(batch):
    images, masks, box, input_size, original_img_size = zip(*batch)  # Rozbal jednotlivé prvky
    return images, masks, box, list(input_size), list(original_img_size)  # Vrátí input_sizes jako seznam n-tic

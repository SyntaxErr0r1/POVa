...
def plot_results(sam_model, val_loader, epoch):
    ...
    for idx in range(batch_size):
        ...
        pred_mask = (binary_masks[idx].cpu().detach().numpy().squeeze() > 0.5).astype(np.uint8)
    ...
    plt.tight_layout()
    plt.savefig(f"results_batch_{epoch}.png")
 
def plot_results2(sam_model, val_loader, epoch):
    images, masks, box = next(iter(val_loader))  # Get a random batch
    images, masks, box = images.to(device), masks.to(device), box.to(device)
    ...
    image = images[random_idx].cpu().detach().permute(1, 2, 0).numpy() * 0.5 + 0.5
    image = np.clip(image, 0, 1)
    ...
    plt.savefig(f"results_{epoch}.png")
###################################
## Parse the arguments
...
for epoch in range(cfg['TRAIN']['EPOCHS']):
    ...
    #for images, masks, box in train_loader:
        for images, masks, box in train_bar:
            ...
            upscaled_masks = sam_model.postprocess_masks(low_res_masks, target_size, target_size).to(device)
            binary_mask = normalize(threshold(torch.sigmoid(upscaled_masks), 0.0, 0)).to(device)

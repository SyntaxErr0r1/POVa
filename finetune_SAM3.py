...
## SAM Model imports
from sam.segment_anything_ori import SamPredictor, sam_model_registry

## Dataset imports
from seg_dataset_SAM3 import SegmentationDataset, collate_fn
###################################
...
for epoch in range(cfg['TRAIN']['EPOCHS']):
    ...
    #for images, masks, box in train_loader:
        for images, masks, box, input_size, original_img_size in train_bar:
            ...
            upscaled_masks = sam_model.postprocess_masks(low_res_masks, input_size, original_img_size).to(device)
            binary_mask = normalize(threshold(torch.sigmoid(upscaled_masks), 0.5, 0)).to(device)
            print(f"upscaled_masks:{upscaled_masks.shape}\nbinary_mask: {binary_mask.shape}")
    ...
    with torch.no_grad():
        with tqdm(val_loader, desc=f"Epoch {epoch + 1}/{cfg['TRAIN']['EPOCHS']}", unit="batch") as val_bar:
            for images, masks, box, input_size, original_size in val_bar:
                images, masks, box = images.to(device), masks.to(device), box.to(device)
                image_embedding = sam_model.image_encoder(images)
                sparse_embeddings, dense_embeddings = sam_model.prompt_encoder(
                    points=None,
